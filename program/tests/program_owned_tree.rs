...
async fn program_owned_tree() {
    ...
    // Create RuleSet
    // --------------------------------
    // Generate a Merkle tree containing lpl-token-auth-rules as a leaf.
    let tree = create_test_merkle_tree_from_one_leaf(&lpl_token_auth_rules::ID, 4);
    ...
    // --------------------------------
    // Our authority key is going to be an account owned by the lpl-token-auth-rules program.
